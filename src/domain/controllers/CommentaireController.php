...
final class CommentaireController
{
    ...
    public function PostMessage(ServerRequestInterface $request, array $args): ResponseInterface
    {
        ...
        if ($contenu['sucess'] == true) {
            ...
            if ($resultMail['mail_concerned'] != null) {
                foreach ($resultMail['data'] as $valeurs) {
                    $options = $options->setToken($valeurs['token'])->setType($valeurs['type_device']);
                    $broker->Produce($options, $metadata);
                }
            }
            if ($resultMobile['mobile_concerned']) {
                foreach ($resultMobile['data'] as $valeurs) {
                    $options = $options->setToken($valeurs['token'])->setType($valeurs['type_device']);
                    $broker->Produce($options, $metadata);
                }
            }
