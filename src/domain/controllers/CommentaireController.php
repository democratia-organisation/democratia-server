...
final class CommentaireController
{
    ...
    {
        return $this->api->execute(array_values($args), 'SELECT c.id_commentaire, contenu_message, horodatage, nb_signalement, prenom_internaute, nom_internaute, id_role FROM commentaire c INNER JOIN internaute i ON c.id_internaute = i.id_internaute INNER JOIN infos_membre ifo ON c.id_internaute = ifo.id_internaute INNER JOIN role r ON ifo.id_role = r.id_role WHERE c.id_groupe = uuid_to_bin(?) AND id_proposition = ?');
    }
}
